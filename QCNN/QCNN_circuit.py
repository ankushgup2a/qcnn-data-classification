...
# Quantum Circuits for Pooling circuit 2
def pooling2_layer1(V):
    for i in range(0, 8, 2):
        V(wires=[i + 1, i])
def pooling2_layer2(V):
    V(wires=[2,0])
    V(wires=[6,4])
def pooling2_layer3(V):
    V(wires=[0,4])

# Quantum Circuits for Pooling circuit 3 = (\phi, \theta, \omega)
def pooling3_layer1(V, *params):
    for i in range(0, 8, 2):
        V(*params, wires=[i + 1, i])
def pooling3_layer2(V, *params):
    V(*params, wires=[2,0])
    V(*params, wires=[6,4])
def pooling3_layer3(V, *params):
    V(*params, wires=[0,4])
...
def QCNN_structure_pool2(U, params, U_params):
    param1 = params[0:U_params]
    param2 = params[U_params: 2 * U_params]
    param3 = params[2 * U_params: 3 * U_params]
    ...
    # Pooling Ansatz1 is used by default
    conv_layer1(U, param1)
    pooling2_layer1(unitary.Pooling_ansatz2)
    conv_layer2(U, param2)
    pooling2_layer2(unitary.Pooling_ansatz2)
    conv_layer3(U, param3)
    pooling2_layer3(unitary.Pooling_ansatz2)

def QCNN_structure_pool3(U, params, U_params):
    param1 = params[0:U_params]
    param2 = params[U_params: 2 * U_params]
    param3 = params[2 * U_params: 3 * U_params]
    param4 = params[3 * U_params: 3 * U_params + 3]
    param5 = params[3 * U_params + 3: 3 * U_params + 6]
    param6 = params[3 * U_params + 6: 3 * U_params + 9]

    # Pooling Ansatz1 is used by default
    conv_layer1(U, param1)
    pooling3_layer1(unitary.Pooling_ansatz1, param4)
    conv_layer2(U, param2)
    pooling3_layer2(unitary.Pooling_ansatz1, param5)
    conv_layer3(U, param3)
    pooling3_layer3(unitary.Pooling_ansatz1, param6)
